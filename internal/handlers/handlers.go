...
import (
	...
	"github.com/chingsley/bookings/internal/repository/dbrepo"
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	if !form.Valid() {
		...
		// http.Error(w, "my own error message", http.StatusSeeOther) // remove this, it would render the frontend in raw format
		render.Template(w, r, "make-reservation.page.tmpl", &models.TemplateData{
			Form: form,
			Data: data,
		})
...
// PostShowLogin - handles user login
...
// AdminNewReservations shows all new (unproceesed) reservations
...
// AdminShowReservation displays the details of one reservation
...
// AdminPostShowReservation handles reservation update
func (m *Repository) AdminPostShowReservation(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		m.App.Session.Put(r.Context(), "error", "Sorry, something went wrong")
		http.Redirect(w, r, fmt.Sprintf("/admin/reservations/%s/%d", src, res.ID), http.StatusSeeOther)
		return
	}
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		...
		// get all the restrictions fro the current room
		...
		for _, y := range restrictions {
			if y.ReservationID > 0 {
				// it's a reservation (ie. room has been booked/reserved)
				for d := y.StartDate; d.After(y.EndDate) == false; d = d.AddDate(0, 0, 1) {
					reservationMap[d.Format("2006-01-2")] = y.ReservationID
				}
			} else {
				// it's a block (i.e room is not booked, but is blocked by admin)
...
func (m *Repository) AdminProcessReservation(w http.ResponseWriter, r *http.Request) {
	...
	m.App.Session.Put(r.Context(), "flash", "Suuccessfully marked reservation as processed")
	http.Redirect(w, r, fmt.Sprintf("/admin/reservations-%s", src), http.StatusSeeOther)
}
