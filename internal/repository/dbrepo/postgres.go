...
func (m *postgresDBRepo) Authenticate(email, testPasssword string) (int, string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	...
	err = bcrypt.CompareHashAndPassword([]byte(hashedPassword), []byte(testPasssword))
...
// AllNewReservations returns a slice of all un-processed reservations
...
// GetReservationByID gets reservation by reservation ID
...
// UpdateProcessedForReservation updates a reservation in the database
func (m *postgresDBRepo) UpdateProcessedForReservation(id int, processed int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	query := `update reservations reservations  set processed = $1 where id = $2`
